...
// Basis-URL (Verzeichnis von /intern)
$scheme = (!empty($_SERVER['HTTPS']) && $_SERVER['HTTPS'] !== 'off') ? 'https' : 'http';
$host = $_SERVER['HTTP_HOST'] ?? 'localhost';
$configBasePath = rtrim(dirname($_SERVER['SCRIPT_NAME'] ?? '/'), '/');
...
if ($method === 'GET') {
    // akzeptiere id oder share als Parameter
    $id = $_GET['id'] ?? ($_GET['share'] ?? null);
    if (!$id) {
...
$payload['share'] = [
    'id' => $id,
    'createdAt' => $createdAt,
    // Aufruf über Haupt-App mit target=intern
    'viewUrl' => $configBaseUrl . '/?share=' . $id . '&target=intern',
    'rawUrl' => $configBaseUrl . '/' . $id . '.json'
];
